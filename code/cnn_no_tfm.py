...
# -------------------- Setup Paths --------------------
# Build all paths relative to the home directory.
HOME_DIR = os.path.expanduser("~")  # e.g. "/Users/okalova" or "/home/okalova"
BASE_DIR = os.path.join(HOME_DIR, "sleep", "STAT-4830-GOALZ-project", "data", "sleep-edf-database-expanded-1.0.0")
# In this version, we do not write results to disk.
...
def detect_spindles(epoch_data, spindle_band=(11, 16), threshold_factor=2.5):
    ...
    Detect spindles in each epoch (using the first channel, assumed to be EEG):
      ...
      2. Filter with sosfiltfilt.
      3. Compute the absolute envelope.
      4. Set threshold = threshold_factor * mean(envelope).
      5. Count peaks above threshold.
    ...
    for epoch in epoch_data:  # epoch shape: (n_channels, n_times)
        eeg = epoch[0]  # use the first channel (EEG Fpz-Cz)
...
# -------------------- Dataset Classes --------------------
# These load raw data directly (without pre-saving NPZ files)
...
class RawSleepDataset(Dataset):
    ...
    def __init__(self, base_dir):
        ...
        all_labels = []
        for sub in SUBFOLDERS:
            psg_files = glob.glob(os.path.join(base_dir, sub, '*-PSG.edf'))
            for psg_file in psg_files:
                ...
                try:
                    ...
                    print(f"Error processing {psg_file}: {e}")
        if len(all_data) == 0:
            raise ValueError("No raw data was loaded; please check your BASE_DIR and file structure.")
        self.data = np.concatenate(all_data, axis=0)
        self.labels = np.concatenate(all_labels, axis=0)
    def __len__(self):
        return self.data.shape[0]
    def __getitem__(self, idx):
        # Consistently add a singleton dimension so that shape is (n_channels, 1, n_times)
...
class SelfSupervisedSleepDataset(Dataset):
    """
    For self-supervised pretraining: loads raw EDF epochs (using RawSleepDataset) and returns
    ...
    def __getitem__(self, idx):
        epoch = self.data[idx]
        # Ensure consistent shape: (n_channels, n_times) -> add singleton dimension.
        if len(epoch.shape) == 2:
            epoch = np.expand_dims(epoch, axis=0)
        # To be consistent with RawSleepDataset, also add an extra singleton dimension.
        epoch = torch.tensor(epoch, dtype=torch.float32).unsqueeze(1)
        aug1 = augment_epoch(epoch)
        aug2 = augment_epoch(epoch)
        return aug1, aug2

def augment_epoch(epoch, noise_std=0.05):
    # Simple augmentation: add Gaussian noise. Operates element-wise.
    noise = torch.randn_like(epoch) * noise_std
    return epoch + noise

# -------------------- Model Definitions --------------------
# We remove the domain adaptation branch for now.
...
# For supervised training, we only use encoder and classifier.
...
def train_self_supervised(epochs=5, batch_size=64):
    # Load self-supervised dataset (raw EDF loading on-the-fly)
    dataset = SelfSupervisedSleepDataset(BASE_DIR)
    # Determine in_channels from the first sample; expected shape after unsqueeze: (n_channels, 1, n_times)
    sample, _ = dataset[0]
...
def train_supervised(epochs=10, batch_size=32):
    # Use RawSleepDataset (supervised training) - note: domain adaptation removed.
    ...
    # Post-processing evaluation: here we use median smoothing
